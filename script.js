...
function multiplyInputs () {
  ...
  // Checks if both inputs are numbers equal to 0, both positive or both negative.
  if (!isNaN(num1) && !isNaN(num2)) {
    if (((num1 >= 0) && (num2 >= 0)) || ((num1 <= 0) && (num2 <= 0))) {
      // Set both numbers to positive (negative * negative = positive)
      num1 = Math.abs(num1)
      num2 = Math.abs(num2)
    ...
    // Otherwise, one number must be positive and one negative.
    } else {
      // Set both numbers to positive.
      num1 = Math.abs(num1)
      num2 = Math.abs(num2)
